...
SCLang.prototype.makeConfigFile = function(options, cb) {
  /**
    write options as yaml to a temp file
    and return the path
  **/
  var addPath, scConfig = {}, includePaths, str;
  includePaths = options.includePaths || [];

  // add sc-classes to sclang's compile paths
  if(options.errorsAsJSON) {
    addPath = path.resolve(__dirname, '../sc-classes');
    includePaths.push(addPath);
  }
  scConfig.includePaths = _.uniq(includePaths);
...
SCLang.prototype.boot = function() {
  ...
  function doSpawn(extraOptions) {
    var commandLineOptions = _.clone(self.options);
    if(extraOptions) {
      commandLineOptions = _.extend(commandLineOptions, extraOptions);
    }
    self.process = spawn(execPath, self.args(commandLineOptions),
      {
        cwd: path.dirname(execPath)
      });

    // temporary listener till booted
    self.addListener('state', resolver);
  ...
  if(this.options.sclang_conf) {
    try {
      sclang_conf = yaml.safeLoad(fs.readFileSync(untildify(this.options.sclang_conf), 'utf8'));
    } catch (e) {
      this.log.err(e);
      throw 'Cannot open or read specified sclang_conf' + this.options.sclang_conf;
    }
    // merge
    this.options.includePaths = _.union(sclang_conf.includePaths || [], this.options.includePaths || []);
    this.options.excludePaths = _.union(sclang_conf.excludePaths || [], this.options.excludePaths || []);
    this.options.postInlineWarnings = _.isUndefined(this.options.postInlineWarnings) ? sclang_conf.postInlineWarnings : this.options.postInlineWarnings;
  }

  // read default config and merge the includePaths
  if(this.options.errorsAsJSON ||
    this.options.includePaths ||
    this.options.excludePaths) {
      this.makeConfigFile(this.options, function(configPath) {
        doSpawn({config: configPath});
      });
  } else {
    doSpawn();
  }
